      ...
      showDialog(
          ...
          builder: (_) {
            return DialogLoading(
              title: widget.locale == const Locale('en') ? 'Loading' : 'ກຳລັງໂຫຼດ',
            );
          });
      if (result != null) {
        _processAuthTokenResponse(result).then((value) {
          if (mounted) {
            setState(() {});
          }
        });
  ...
  Future<void> _processAuthTokenResponse(AuthorizationTokenResponse response) async {
    try {
      ...
      await Future.wait([
        ...
        PreferenceInfo().setRefreshToken(_refreshToken),
                            ...
                            if (tfDialogLoginPhoneNumber.text == '2077710008') {
                              ...
                              if (tfDialogLoginPhoneNumber.text.length == 8) {
                                ...
                                errorDialog(
                                    context,
                                    errorTexthead,
                                    widget.locale == const Locale('en') ? 'Your phone number was wrong' : "ຂໍອະໄພ ເບີໂທຂອງທ່ານບໍ່ຖືກຕ້ອງ",
                                    errorbtn,
                                    fontText);
                              }
